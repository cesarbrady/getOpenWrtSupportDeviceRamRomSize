...
import (
	...
	wwwhoisgo "github.com/likexian/whois-go"
...
func getDomainWhoisInfo(domain string) string {
	...
	if keyInMap(suffix, domainSuffixWhoisServerMap) {
		...
		if len(s) == 0 {
			_, fn, line, _ := runtime.Caller(0)
			panic(filepath.Base(fn) + ":" + strconv.Itoa(line-2) + " >> 找不到顶级域" + suffix + "的whois服务器 >> " + fmtDebugStack(string(debug.Stack())))
		}
		dws = strStrip(s[0][1])
...
func (m *javascriptVMStruct) get(variableName string) string {
	...
	if valueStr == "undefined" {
		_, fn, line, _ := runtime.Caller(0)
		panic(filepath.Base(fn) + ":" + strconv.Itoa(line-1) + " >> 变量" + variableName + "未定义 >> " + fmtDebugStack(string(debug.Stack())))
	}
	return valueStr
}
